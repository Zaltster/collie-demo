...
def test_final_approach_calibration_updates_only_while_disarmed() -> None:
    async def scenario() -> None:
        avoidance = FakeAvoidance()
        runtime = CollieRuntime(
            camera=StaticFruitCamera(),
        ...
        await runtime.start()
        try:
            status = await runtime.configure_final_approach(1.25, 0.08)

            assert status["mission"]["final_approach_duration_s"] == 1.25
            assert status["mission"]["final_approach_mps"] == 0.08
            assert (
                status["mission"][
            ...
            await runtime.navigation_arm(NAVIGATION_ARM_CONFIRMATION)
            try:
                await runtime.configure_final_approach(1.5, 0.08)
            except RuntimeCommandError as exc:
                assert "stop the mission and all motion" in str(exc)
            else:
                raise AssertionError("calibration changed while motion was armed")
...
def test_final_approach_calibration_enforces_hard_limits() -> None:
    async def scenario() -> None:
        runtime = CollieRuntime(
            camera=StaticFruitCamera(),
            controller=ApproachController(),
        ...
        for duration_s, forward_mps, message in (
            (3.01, 0.10, "duration"),
            (1.0, 0.04, "speed"),
            (2.1, 0.15, "commanded distance"),
        ):
            try:
                await runtime.configure_final_approach(
                    duration_s, forward_mps
                )
            except RuntimeCommandError as exc:
                assert message in str(exc)
            else:
                raise AssertionError(f"unsafe {message} was accepted")
